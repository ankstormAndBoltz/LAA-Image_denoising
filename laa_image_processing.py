...
def rgb_svd_denoise(noisy_rgb: np.ndarray, k: int = 30) -> np.ndarray:
    """Applies SVD denoising to each RGB channel independently."""
    channels = []
    for c in range(3):
        ch_denoised = svd_denoise(noisy_rgb[:, :, c], k=k)
        channels.append(ch_denoised)
    return np.stack(channels, axis=-1)


def rgb_gaussian_denoise(rgb_img: np.ndarray, sigma: float = 1.5) -> np.ndarray:
    """Applies Gaussian blur to each RGB channel independently."""
    channels = []
    for c in range(3):
        ch_blurred = gaussian_filter_denoise(rgb_img[:, :, c], sigma=sigma)
        channels.append(ch_blurred)
    return np.stack(channels, axis=-1)
...
def process_uploaded_image(image_path: str) -> None:
    ...
    # Apply both methods directly on the RGB image using our new wrappers
    svd_denoised   = rgb_svd_denoise(rgb_img, k=30)
    gauss_denoised = rgb_gaussian_denoise(rgb_img, sigma=1.5)

    print("  SVD denoising (RGB)  (k=30)       : done")
    print("  Gaussian filter (RGB) (sigma=1.5) : done")
...
# ─────────────────────────────────────────────────────────────────────────
    ...
    # Get dimensions from the denoised image instead of original
    h_den, w_den = svd_denoised.shape[:2]

    # 1.5× scale-up  (INTER_LINEAR = bilinear interpolation)
    scaled_up = cv2.resize(svd_denoised, (int(w_den * 1.5), int(h_den * 1.5)),
                           interpolation=cv2.INTER_LINEAR)
    print(f"  Scaled UP  (1.5×) : {svd_denoised.shape} → {scaled_up.shape}")

    # 0.5× scale-down  (INTER_AREA = better for shrinking)
    scaled_dn = cv2.resize(svd_denoised, (max(1, int(w_den * 0.5)), max(1, int(h_den * 0.5))),
                           interpolation=cv2.INTER_AREA)
    print(f"  Scaled DOWN (0.5×): {svd_denoised.shape} → {scaled_dn.shape}")
    ...
    # Note: Removed cmap="gray" because these are now RGB images
    axes[1][0].imshow(svd_denoised)
    axes[1][0].set_title("SVD Denoised (k=30)\nAₖ = Uₖ Σₖ Vₖᵀ", color=ACCENT)
    axes[1][0].axis("off")

    axes[1][1].imshow(gauss_denoised)
    axes[1][1].set_title("Gaussian Denoised\nG * A  (linear convolution)", color=ACCENT)
    axes[1][1].axis("off")

    # Third panel in row 2: changed to rgb_img for an apples-to-apples color comparison
    axes[1][2].imshow(rgb_img)
    axes[1][2].set_title("Original Color (reference)", color=MUTED)
    axes[1][2].axis("off")

# Row 3: Scaled Up | Scaled Down | Rotated 45°
    axes[2][0].imshow(scaled_up) # Removed cmap="gray"
    axes[2][0].set_title(f"Scaled UP 1.5×\n{scaled_up.shape[1]}×{scaled_up.shape[0]}",
                         color=PURPLE)
    axes[2][0].axis("off")

    axes[2][1].imshow(scaled_dn) # Removed cmap="gray"
    ...
    saves = [
            ...
            ("svd_denoised.png",      svd_denoised,    "RGB"),
            ("gaussian_denoised.png", gauss_denoised,  "RGB"),
            ("scaled_up.png",         scaled_up,       "RGB"), # Changed to RGB
            ("scaled_down.png",       scaled_dn,       "RGB"), # Changed to RGB
